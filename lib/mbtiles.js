...
function MBTiles(uri, callback) {
    const response = promisify(global.Promise, callback)
...
MBTiles.prototype.write = function(table, id, row, callback) {
    ...
    if(++this._pending >= this._batchSize)
...
MBTiles.prototype.getGeocoderData = function(type, shard, callback) {
    ...
    this._db.get('SELECT data FROM geocoder_data WHERE type = ? AND shard = ?', type, shard, function(err, row) {
