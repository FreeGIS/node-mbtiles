...
fs.readdirSync(__dirname + '/fixtures/images/').forEach(function(file) {
    ...
    tape('tile ' + coords.join('/'), function(assert) {
        loaded.plain_1.getTile(coords[0] | 0, coords[1] | 0, coords[2] | 0, function(err, result) {
            if (err) throw err;
            assert.deepEqual(result.tile, fs.readFileSync(__dirname + '/fixtures/images/' + file));
            assert.equal(result.headers['Content-Type'], 'image/png');
            assert.ok(!isNaN(Date.parse(result.headers['Last-Modified'])));
            assert.ok(/\d+-\d+/.test(result.headers['ETag']));
            assert.end();
        });
    });
...
fs.readdirSync(__dirname + '/fixtures/grids/').forEach(function(file) {
    ...
    coords[2] = Math.pow(2, coords[0]) - 1 - coords[2];
    tape('grid ' + coords.join('/'), function(assert) {
        loaded.plain_2.getGrid(coords[0] | 0, coords[1] | 0, coords[2] | 0, function(err, result) {
            if (err) throw err;
            assert.deepEqual(JSON.stringify(result.grid), fs.readFileSync(__dirname + '/fixtures/grids/' + file, 'utf8'));
    ...
    tape('grid alt ' + coords.join('/'), function(assert) {
        loaded.plain_4.getGrid(coords[0] | 0, coords[1] | 0, coords[2] | 0, function(err, result) {
            if (err) throw err;
            assert.deepEqual(JSON.stringify(result.grid), fs.readFileSync(__dirname + '/fixtures/grids/' + file, 'utf8'));
            assert.equal(result.headers['Content-Type'], 'text/javascript');
            assert.ok(!isNaN(Date.parse(result.headers['Last-Modified'])));
